...
func PrefetchList(proxyURL, bucket string, fileslist []string, wait bool, deadline time.Duration) error {
	...
	return doListRangeCall(proxyURL, bucket, cmn.CloudBs, cmn.ActPrefetch, http.MethodPost, prefetchMsg)
}
...
func EvictList(proxyURL, bucket string, fileslist []string, wait bool, deadline time.Duration) error {
	...
	return doListRangeCall(proxyURL, bucket, cmn.CloudBs, cmn.ActEvictObjects, http.MethodDelete, evictMsg)
}
